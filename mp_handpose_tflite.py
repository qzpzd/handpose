...
class MPHandPoseTFLite:
    ...
    def _cropAndPadFromPalm(self, image, palm_bbox, for_rotation=True):
        """从手掌边界框裁剪和填充图像"""
        # shift bounding box - 先shift
    ...
    def _preprocess(self, image, palm):
        ...
        if isinstance(palm, dict):
            palm_bbox = palm['bbox'].reshape(2, 2)
            palm_landmarks = palm['landmarks'].reshape(7, 2)
            palm_data = np.concatenate([palm_bbox.flatten(), palm_landmarks.flatten()])
        else:
            palm_data = palm
        ...
        palm_bbox -= pad_bias
        palm_landmarks = palm_data[4:18].reshape(7, 2) - pad_bias
        ...
        crop, rotated_palm_bbox, _ = self._cropAndPadFromPalm(rotated_image, rotated_palm_bbox, False)
    ...
    def _postprocess(self, blob, rotated_palm_bbox, angle, rotation_matrix, pad_bias):
        ...
        # [131]: confidence
        return np.r_[bbox.reshape(-1), landmarks.reshape(-1), rotated_landmarks_world.reshape(-1), handedness, conf]

    def infer(self, image, palm):
        """执行推理"""
        try:
            # Preprocess
            result = self._preprocess(image, palm)
            if result is None:
                return None
            
            input_blob, rotated_palm_bbox, angle_deg, rotation_matrix, pad_bias = result
            ...
            if "Empty image" in str(e):
                return None
            else:
                raise

        # Postprocess
        results = self._postprocess(output_blob, rotated_palm_bbox, angle_deg, rotation_matrix, pad_bias)
        return results