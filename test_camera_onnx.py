...
def main():
    print("=" * 80)
    print("ONNX 格式模型摄像头测试")
    print("=" * 80)
    
    # 模型路径
    palm_det_path = f"{PalmDet_MODEL}.onnx"
    hand_landmark_path = f"{HandLandmark_MODEL}.onnx"
    ...
    print("\n加载 ONNX 模型...")
    try:
        palm_detector = MPPalmDet(palm_det_path, use_gpu=True)
        hand_detector = MPHandPose(hand_landmark_path, use_gpu=True)
        print("✓ 模型加载成功")
    ...
    try:
        while True:
            ...
            # 显示帧
            cv2.imshow("ONNX Camera Test", frame)
